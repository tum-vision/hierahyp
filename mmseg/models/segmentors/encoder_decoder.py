...
class EncoderDecoder(BaseSegmentor):
    ...
    def _auxiliary_head_forward_train(self, x, img_metas, gt_semantic_seg):
        ...
        if isinstance(self.auxiliary_head, nn.ModuleList):
            ...
            loss_aux = self.auxiliary_head.forward_train(
                x, self.offsets,
                self.normals, self.curvature,
                img_metas, gt_semantic_seg, self.train_cfg)
    ...
    def simple_test(self, img, img_meta, rescale=True, ):
        ...
        seg_logit, cur_cert_logit =  self.inference(img, img_meta, rescale) 

        return cur_cert_logit
        if self.test_cfg['is_hiera']:

            
                
            if seg_logit.size(1)==12:
            ...
            seg_logit, _ = self.inference(img, img_meta, rescale)
