...
class LrUpdaterHook(Hook):
    ...
    def _set_lr(self, runner, lr_groups):
        for opti in runner.optimizer.optimizers:
            if isinstance(opti, dict):
                for k, optim in opti.items():
                    for param_group, lr in zip(optim.param_groups, lr_groups[k]):
                        param_group['lr'] = lr
            else:
                for param_group, lr in zip(opti.param_groups,
                                           lr_groups):
                    param_group['lr'] = lr
    ...
    def get_regular_lr(self, runner):
        #print('in lr_updater.py, GET_REGULAR_LR')
        if isinstance(runner.optimizer, dict):
            lr_groups = {}
    ...
    def before_run(self, runner):
        # NOTE: when resuming from a checkpoint, if 'initial_lr' is not saved,
        # it will be set according to the optimizer params
        #print('in lr_updater.py, BEFORE_RUN')
        for op in runner.optimizer.optimizers:
            if isinstance(op, dict):
                self.base_lr = {}
                for k, optim in op.items():
                    for group in optim.param_groups:
                        group.setdefault('initial_lr', group['lr'])
                    _base_lr = [
                        group['initial_lr'] for group in optim.param_groups
                ...
                for group in op.param_groups:
                    group.setdefault('initial_lr', group['lr'])
                self.base_lr = [
                    group['initial_lr'] for group in op.param_groups
                ]            

    def before_train_epoch(self, runner):
        #print('in lr_updater.py, BEFORE_TRAIN_EPOCH')
    ...
    def before_train_iter(self, runner):
        #print('in lr_updater.py, BEFORE_TRAIN_ITER')
